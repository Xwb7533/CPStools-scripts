    ...
    if not all_list:
                ...
                if sequences[start1 - 1] == replace_dit[sequences[end2]]:
                        ...
                        if sequences[new_start] == replace_dit[sequences[0]]:
                            ...
                            if (start2 - end1) <= (new_start - new_end):
                                result = (
                                    ...
                                    f"SSC:{end1}-{start2 + 1}\nIRa:{start2 + 2}-{len(sequences)},1-{new_end}"
                                )
                            # SSC在前
                            else:
                                result = (
                                    f"LSC:{end1}-{start2 + 1}\nIRb:{start2 + 2}-{len(sequences)},1-{new_end}\n"
                    ...
                    # 不跨区域
                        ...
                        if (start2 - end1) <= (len(sequences) - new_end + new_start):
                            result = (
                                f"LSC:{new_end + 1}-{len(sequences)},1-{new_start + 1}\nIRb:{new_start + 2}-{end1 - 1}\n"
                                f"SSC:{end1}-{start2 + 1}\nIRa:{start2 + 2}-{new_end}"
                            )
                        # SSC在前
                        else:
                            result = (
                                f"LSC:{end1}-{start2 + 1}\nIRb:{start2 + 2}-{new_end}\n"
                                f"SSC:{new_end + 1}-{len(sequences)},1-{new_start + 1}\nIRa:{start1 + 2}-{end1 - 1}"
                            )
                # 停止延伸
                else:
                    # LSC在前
                    if (start2 - end1) <= (len(sequences) - end2 + start1):
                        result = (
                            f"LSC:{end2 + 1}-{len(sequences)},1-{start1 + 1}\nIRb:{start1 + 2}-{end1 - 1}\n"
                            f"SSC:{end1}-{start2 + 1}\nIRa:{start2 + 2}-{end2}"
                        )
                    # SSC在前
                    else:
                        result = (
                            f"LSC:{end1}-{start2 + 1}\nIRb:{start2 + 2}-{end2}\n"
                            f"SSC:{end2 + 1}-{len(sequences)},1-{start1 + 1}\nIRa:{start1 + 2}-{end1 - 1}"
                        )
            # 直接跨区域
            else:
                # 延伸
                ...
                # 不延伸
                    ...
                    if (start2 - end1) <= start1:
                        ...
                        result = (
                            f"LSC:{end1}-{start2 + 1}\nIRb:{start2 + 2}-{end2}\n"
                            f"SSC:{end2 + 1}-{len(sequences)},1-{start1}\nIRa:{start1 + 1}-{end1 - 1}"
                        )
        ...
        # IR 区域在1处
            ...
            if sequences[-1] == replace_dit[sequences[end2]]:
                ...
                if (start2 - end1) <= (new_start - new_end):
                    result = (
                        f"LSC:{new_end + 1}-{new_start + 1}\nIRb:{new_start + 2}-{len(sequences)},1-{end1 - 1}\n"
                        f"SSC:{end1}-{start2 + 1}\nIRa:{start2 + 2}-{end2}"
                    )
                # SSC在前
                else:
            ...
            # 不跨区域延伸
                ...
                if (start2 - end1) <= (len(sequences) - end2 + start1):
                    result = (
                        f"LSC:{end2 + 1}-{len(sequences)}\nIRb:{start1 + 1}-{end1 - 1}\n"
                        f"SSC:{end1}-{start2 + 1}\nIRa:{start2 + 2}-{end2}"
                    )
                else:
                    result = (
                        f"LSC:{end1}-{start2 + 1}\nIRb:{start2 + 2}-{end2}\n"
                        f"SSC:{end2 + 1}-{len(sequences)}\nIRa:{start1 + 1}-{end1 - 1}"
                    )
        return result
...
def main():
    ...
    parser.add_argument('-i', '--input_file', help='fasta format file', required=True)
